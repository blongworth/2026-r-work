...
```{r, include=FALSE}
lter <- read.csv("data/ar61b_ctd_cast_data.csv")
library(gapminder)
```
...
Let's start off building an example using the NES-LTER data from earlier.
...
```{r ggplot-with-aes, message=FALSE, fig.alt="Plotting area with axes for a scatter plot of temperature vs depth, with no data points visible."}
ggplot(data = lter, mapping = aes(x = depth, y = temperature))
```

Here we told `ggplot` we want to plot the "depth" column of the
LTER data frame on the x-axis, and the "temperature" column on the
y-axis. Notice that we didn't need to explicitly pass `aes` these
columns (e.g. `x = lter[, "depth"]`), this is because
...
```{r lifeExp-vs-gdpPercap-scatter, message=FALSE, fig.alt="Scatter plot of temperature vs depth, now showing the data points."}
ggplot(data = lter, mapping = aes(x = depth, y = temperature)) +
  geom_point()
```
...
Using a scatterplot probably isn't the best for visualizing change over depth.
Instead, let's tell `ggplot` to visualize the data as a line plot:

```{r lifeExp-line}
ggplot(
  data = lter,
  mapping = aes(x = depth, y = temperature, color = station)
) +
  geom_line()
```
...
However, the result doesn't look quite as we might have expected: it seems to be jumping around a lot in each station. Let's try to separate the data for each time we've visited a station, plotting one line for each cast:

```{r lifeExp-line-by}
ggplot(
  data = lter,
  mapping = aes(x = depth, y = temperature, group = cast, color = station)
) +
  geom_line()
```

We've added the **group** *aesthetic*, which tells `ggplot` to draw a line for each
country. This doesn't clear things up too much because most repeat visits 
were shallow surface casts.
...
ggplot(data = lter, mapping = aes(x = depth, y = temperature, group = cast)) +
  geom_line(mapping = aes(color = station)) +
  geom_point()
```
...
So far, we've seen how to use an aesthetic (such as **color**) as a *mapping* to a variable in the data. For example, when we use `geom_line(mapping = aes(color=station))`, ggplot will give a different color to each station. But what if we want to change the color of all lines to blue? You may think that `geom_line(mapping = aes(color="blue"))` should work, but it doesn't. Since we don't want to create a mapping to a specific variable, we can move the color specification outside of the `aes()` function, like this: `geom_line(color="blue")`.
...
Currently it's hard to see the relationship between the points due to some overlap
in values near the surface. We'd also like to plot depth increasing as we go downward
on the x axis, so this looks like what we expect for an oceanographic plot.
We can change the scale of the depth units with
the *scale* functions. These control the mapping between the data values and
visual values of an aesthetic. Here, we'll use `scale_x_reverse()` 
to flip the axis.
We'll use `coord_flip()` to swap the x and y axes. 
...
Notice that we used `geom_point(alpha = 0.5)`. As the previous tip mentioned, using a setting outside of the `aes()` function will cause this value to be used for all points, which is what we want in this case. But just like any other aesthetic setting, *alpha* can also be mapped to a variable in the data. For example, we can give a different transparency to each latitude with `geom_point(mapping = aes(alpha = latitude))`. This might help us with our two populations of temperature in the surface water!
...
Earlier we visualized the change in temperature over depth across all
stations in one plot. Alternatively, we can split this out over multiple panels
...
ggplot(
  data = lter,
  mapping = aes(x = depth, y = temperature, color = as.factor(cast))
) +
  geom_line() +
  scale_x_reverse() +
  coord_flip() +
...
(~). This tells R to draw a panel for each unique value in the station column
of the LTER dataset.
...
"Depth [m]", rather than the column name in the data frame.

We can do this by adding a couple of different layers. The **theme** layer
controls the axis text, and overall text size. Labels for the axes, plot
title and any legend can be set using the `labs` function. Legend titles
are set using the same names we used in the `aes` specification. Thus below
the color legend title is set using `color = "Cast"`, while the title
...
ggplot(
  ...
  facet_wrap(~station) +
  labs(
    x = "Depth [m]", # x axis title
    y = "Temperature [C]", # y axis title
    title = "Figure 1", # main title of figure
    color = "Cast" # title of legend
  ) +
...
The `ggsave()` function allows you to export a plot created with ggplot. You can specify the dimension and resolution of your plot by adjusting the appropriate arguments (`width`, `height` and `dpi`) to create high quality graphics for publication. In order to save the plot from above, we first assign it to a variable `td_plot`, then tell `ggsave` to save that plot in `png` format to a directory called `results`. (Make sure you have a `results/` folder in your working directory.)
...
ggsave(
  filename = "results/ar61b_temp_depth.png",
  plot = td_plot,
  width = 12,
  height = 10,
  dpi = 300,
